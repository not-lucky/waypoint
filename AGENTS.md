...
- **Errors** — The v1 client envelope (`buildClientErrorEnvelope`, `src/errors/envelope.js:31`) wraps the upstream's raw fields verbatim. Required fields: `code` (default `'upstream_error'`), `message` (default `'Request failed'`), `httpStatus`. Optional, conditionally spread: `type`, `provider`, `retryAfterSeconds`, `upstreamCode`, `upstreamBody`, `details`. Stream failures use `BaseController.emitStreamError` with `formatOpenAiSseError` (emits `data: {JSON}\n\n` plus optional `data: [DONE]\n\n`) or `formatAnthropicSseError` (emits `event: error\ndata: {JSON}\n\n`).
...
| `src/errors/envelope.js` | v1 client envelope shape + SSE error formatters. |
...
- **CI** — `.github/workflows/ci.yml` runs `npm ci` then `npm run lint` then `npm test` on a Node matrix of `20, 22, 24, 26` for pushes and pull requests against `master`. The local `ci` script chains the same two commands with echo markers.
