...
describe('Abort and Request Cancellation Tests', () => {
  ...
  // Edge Case: Verify backward compatibility with mockReq.res 'close' binding
  it('assert: client disconnect via rawReq.res triggers abort', async () => {
    let resCloseCallback = null;
    const mockRes = {
      on: (event, cb) => {
        if (event === 'close') resCloseCallback = cb;
      },
    };
    const mockReq = {
      res: mockRes,
    ...
    // Trigger close on the response object
    resCloseCallback();
    expect(mockAdapter.capturedCompletionSignal.aborted).toBe(true);
  });

  // Edge Case: Verify response close does not trigger abort if res.writableEnded is true
  it('assert: res close does not abort if writableEnded is true', async () => {
    let resCloseCallback = null;
    const mockRes = {
      writableEnded: true,
      on: (event, cb) => {
    ...
    // Trigger close on response, but since writableEnded is true, it shouldn't abort
    resCloseCallback();
    expect(mockAdapter.capturedCompletionSignal.aborted).toBe(false);
  });
});
